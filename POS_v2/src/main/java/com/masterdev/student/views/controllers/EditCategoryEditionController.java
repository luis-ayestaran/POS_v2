...
public class EditCategoryEditionController implements Initializable {
	...
	public void editCategory() {
		if(fieldsAreFilledUp()) {
			ProductType newCategory = CategoryEdition.getCategoryEditionController().findCategory(txtEditCategory.getText().trim());
			if(newCategory == null) {
				changeCategoryName(getProductType(), txtEditCategory.getText().trim());
				List<ProductType> categories = CategoryEdition.getCategoryEditionController().showCategoryList();
				...
				CategoryEdition.getCategoryEditionController().selectEditedCategory(getProductType(), getIndex());
				closeStageCompletely();
			} else {
				Dialogs d = new Dialogs();
				d.acceptDialog("Error al editar categoría",
					"El nombre que deseas proporcionar ya está en uso \npor otra categoría.",
					(StackPane)EditCategoryEdition.getStage().getScene().getRoot(), txtEditCategory);
			}
	...
	public void closeStageCompletely() {
		if(EditCategoryEdition.getStage() != null) {
			EditCategoryEdition.getStage().close();
			EditCategoryEdition.setStage(null);
		}
	}
